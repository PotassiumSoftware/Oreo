...
const configPath = path.join(__dirname, "../../config.js"); // Adjust path as necessary
			...
			if (action === "add" || action === "remove") {
				...
				if (action === "add") {
					...
					if (index !== -1) {
						...
						global.config.developers.admin = developerIds; // Update global config

						// Write changes to config file for persistence
						fs.writeFileSync(configPath, `module.exports = ${JSON.stringify(global.config, null, 2)};`);
